...
class ItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Status & Urgency", {"fields": ("status", "urgency", "functional_status")}),
        ("Pricing", {"fields": ("sum_price", "estimated_price", "tva_applied")}),
        ("Sales", {"fields": ("sales_platform",)}),
        (
            "Timestamps",
            {
    ...
    def image_count(self, obj):
        ...
        return mark_safe("".join(
            f'<img src="{img.image.url}" style="width:75px;height:75px;object-fit:cover;border-radius:4px;margin-right:4px;">'
            for img in images
        ))
    ...
    class Media:
        css = {"all": ("inventory/admin_preview.css",)}
...
class SaleHistoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Sale Information", {"fields": ("item", "sold_price", "sold_date")}),
        ("Buyer Details", {"fields": ("buyer_info", "notes")}),
        (
            "System",
            {
                "fields": ("created_at",),
